...
struct WorldTexture
{
	static int tex(int i)
	{
		Bmp b(2048, 2048, 32);
		b.load_float(str("../data/terrain/result.%d.f32", i + 1));
		return ogl_tex_float_bmp(b, GL_LINEAR_MIPMAP_LINEAR, GL_REPEAT);
	}
	static uint get(int i)
	{
		static int t[6] = { tex(0), tex(1), tex(2), tex(3), tex(4), tex(5) };
		return t[i];
	}
};
////////////////////////////////////////////////////////////////////////////////
struct World
{
	...
	static void begin_patch()
	{
		if (vbo == 0)
		{
			...
			shader = Shader("../shader/Patch.Textured");
		}
		...
		loopi(0, 6) ogl_bind(i, WorldTexture::get(i));

		shader.begin();
		shader.setUniform1i("fractal_enabled", gui.screen[0].checkbox["Factal"].checked ? 1 : 0);
		shader.setUniform1i("tx0", 0);
		shader.setUniform1i("tx1", 1);
		shader.setUniform1i("tx2", 2);
		//shader.setUniform1i("tx3", 3);
		//shader.setUniform1i("tx4", 4);
		//shader.setUniform1i("tx5", 5);
	}
